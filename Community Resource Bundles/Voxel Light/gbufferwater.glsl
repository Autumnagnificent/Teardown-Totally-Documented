...
void main(void)
{
	...
	float depthFraction = min(1.0, waterDepth / uVisibility);
...

	{
		...
		for (int i = 0; i < uRingCount; i++)
		{
			...
			float life = ringData.z;
			float age = ringData.w;
			float alpha = (age < 0.1 ? age / 0.1 : 1.0 - (age-0.1) / (life-0.1));
			float size = 1.1;
			float s = clamp(1.0 - length(hitPos.xz - ringData.xy) / size, 0.0, 1.0);
			ringFoam = max(ringFoam, alpha*s*s);
		}
		vec2 remappedXy = xy - (dir.xz*h);
		float foam = texture(uFoamTexture, remappedXy*0.52).r;
		foam *= ringFoam;
		outputColor += vec4(foam);
	}
